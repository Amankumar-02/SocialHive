...
function Search() {
  ...
  const [searchProfileView, setSearchProfileView] = useState([]);
  ...
  const clickEvent = (index) => {
    ...
    setSearchProfileView(postImg[index])
  };
            ...
            <div id="card">
              ...
              >
                <div id="topSearchViewBar">
                  <i className="ri-arrow-left-line text-xl cursor-pointer" onClick={()=>setFullScreen({transform: "scale(0)", backgroundImage: "none"})}></i>
                ...
                <div id="searchImgView">
                        <div id="searchLeftText">
                          ...
                          <div id="searchLeftImg">
                            <img src={searchProfileView.profileDp} alt="" />
                          </div>
                          <p className="text-sm ps-2">{searchProfileView.nameText}</p>
                          <button id="reelBtn" key={searchProfileView.id} onClick={(e) => textToggle(e, searchProfileView.id)}>
                            {searchProfileView.isFollowing ? 'Following' : 'Follow'}
                          </button>
                          {/* <button id="searchViewBtn">Follow</button> */}
                          </div>
                          <div className="pt-3 pb-1">
                            <p className="text-xs">{searchProfileView.caption}</p>
                          </div>
                        ...
                        <div key={searchProfileView.id} className='inline-block' onClick={(e) => likeToggle(e, searchProfileView.id)}>
                        {searchProfileView.isLike ? <i className="ri-heart-fill cursor-pointer text-[#ff0000]"></i> : <i className="ri-heart-line cursor-pointer"></i>}
                        </div>
                        <p>
                          {searchProfileView?.value?.one}
                        {/* {Math.floor(Math.random()*10000)} */}
                        </p>
                        <i className="ri-chat-3-line cursor-pointer"></i>
                        <p>
                          {searchProfileView?.value?.two}
                          {/* {Math.floor(Math.random()*1000)} */}
                        </p>
                        <i className="ri-send-plane-line cursor-pointer"></i>
                        <p>
                          {searchProfileView?.value?.three}
                          {/* {Math.floor(Math.random()*1000)} */}
                          </p>
